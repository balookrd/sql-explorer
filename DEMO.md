...
### 5. Проверка безопасности и аудит-логов (Security & SIEM Audit)
1. **Просмотр структурированного аудита**:
   В терминале просмотрите JSON-события аудита безопасности приложения:
   ```bash
   docker compose -f demo/docker-compose.yaml logs sql-explorer | grep "security.audit"
   ```
   В логах отображаются события успешных входов (`AUTH_LOGIN_SUCCESS`), выполненных запросов (`QUERY_EXECUTED`) и выхода из системы (`AUTH_LOGOUT`).
2. **Защита от перебора паролей (Rate Limiting)**:
   При 5 последовательных попытках входа с неверным паролем на эндпоинт `/api/auth/login` срабатывает лимитер и возвращается HTTP `429 Too Many Requests`.
3. **Персистентный отзыв токенов**:
   При нажатии кнопки «Выйти» токен мгновенно аннулируется в БД и последующие запросы с ним блокируются (`401 Unauthorized`).

### 6. Проверка ИИ-ассистента и SQL Linter
1. **Линтинг и подсветка антипаттернов**:
   Вставьте запрос без `LIMIT` и с `SELECT *`:
   ```sql
   SELECT * FROM tpch.sf1.customer WHERE acctbal > 1000;
   ```
   Нажмите **«🪄 ИИ Анализ»**. Откроется модальное окно с детальным аудитом, а в Monaco Editor появятся цветные маркеры с подсказками.
2. **Пошаговое объяснение запроса**:
   Нажмите **«Объяснить»** — ассистент выведет список задействованных таблиц и шаги выполнения на русском языке.
3. **Оптимизация в 1 клик и валидация повторного применения**:
   Нажмите **«Оптимизировать»** — просмотрите оптимизированный SQL и нажмите **«Применить в редактор»**. Если открыть оптимизацию повторно для уже оптимизированного запроса, ИИ-помощник корректно определит отсутствие необходимости изменений, отобразит статус **«Запрос уже оптимизирован»** и переведет кнопку в неактивное состояние **«Уже применено»**.
4. **Исправление ошибок исполнения**:
   Выполните некорректный для Trino запрос:
   ```sql
   SELECT NVL(custkey, 0) FROM tpch.sf1.customer;
   ```
   После ошибки движка нажмите появившуюся кнопку **«🪄 Исправить с ИИ»** и примените предложенный вариант с функцией `COALESCE`.
